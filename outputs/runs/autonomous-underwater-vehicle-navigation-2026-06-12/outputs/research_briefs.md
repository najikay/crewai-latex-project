...
For each timestep k:
  For each particle i:
    ...
    # Update map (conditional Gaussian or grid)
    m_k^{(i)} = update_map(m_{k-1}^{(i)}, x_k^{(i)}, z_k)
  # Resample if N_eff < threshold
  N_eff = 1 / sum((w_k^{(i)})^2)
  if N_eff < N_thresh:
    resample particles
```
...
Optimize using Gauss-Newton or Levenberg-Marquardt:
  While not converged:
    Linearize: e(X + Delta) ≈ e(X) + J * Delta
...
The foundation of any underwater SLAM system is the mathematical model describing vehicle kinematics and sensor characteristics. For AUV navigation, the state vector typically comprises 12 elements in a 6-degree-of-freedom (6-DOF) representation: three position coordinates (x, y, z), three Euler angles (roll φ, pitch θ, yaw ψ), and their corresponding velocities (u, v, w for linear; p, q, r for angular). This representation follows the Society of Naval Architects and Marine Engineers (SNAME) notation standard.
...
# Rotation update (quaternion or rotation matrix)
R_k = R_{k-1} * exp(omega_k * dt)

# Position update
p_k = p_{k-1} + R_k * v_DVL * dt + 0.5 * R_k * a_k * dt^2

# Velocity update (from DVL)
v_k = v_DVL
...
r = ||m_body||
alpha = atan2(m_body_y, m_body_x)  # azimuth
beta = asin(m_body_z / r)          # elevation
...
| MSIS (Tritech Micron) | Angular resolution | 0.5° | Step size |
...
@article{Hegrenas2016,
  author={O. Hegrenæs and O. Hallingstad},
...
# Step 1: Median filter (3x3 kernel)
...
# Step 3: Speckle reduction (Lee filter)
for each pixel:
...
# Edge detection
Gradient magnitude G = sqrt(G_r^2 + G_theta^2)
Edge map E = non_max_suppression(G)
...
Source: Wang et al. (2023), Table II; Hidalgo-Carrió et al. (2020), Table I.
...
@inproceedings{Hidalgo2020,
  author={J. Hidalgo-Carrió and P. V. K. Borges and S. S. Ge},
...
# Compute landmark position in navigation frame
m_new = g(x_k, z_k) = x_k + R_k * [r*cos(beta)*cos(alpha);
...
|--------|-----------|--------------------------|-------------------|------------|
| EKF-SLAM | O(N²) | 300–500 | 2.8 | Underconfident |
| UKF-SLAM | O(N²) | 200–300 | 2.1 | Better |
...
Importance sampling is the core mechanism for updating particle weights. The proposal distribution π(x_t | x_{1:t-1}, z_{1:t}, u_t) determines how new poses are sampled. The simplest choice is the odometry-based proposal p(x_t | x_{t-1}, u_t), but this ignores the current observation and leads to weight degeneracy. The optimal proposal incorporates the most recent observation: π_opt ∝ p(z_t | x_t, m_{t-1}) · p(x_t | x_{t-1}, u_t). For sonar measurements, this optimal proposal can be approximated by sampling from the motion model and weighting by the acoustic likelihood.
...
For each timestep k:
    For each particle i:
        ...
        # Use motion model to generate candidate poses
        ...
        # Sample from weighted candidates
...
for each cell c in sonar beam:
    ...
    if |range_c - r| < resolution/2:
        # Cell at measured range: likely occupied
        p_occ = 0.7
    elif range_c < r - beam_width/2:
        # Cell between sensor and return: likely free
        p_occ = 0.3
    else:
        # Outside beam: no information
        continue
    
    # Log-odds update
    l_c = log(p_occ / (1 - p_occ))
    l_free = log(0.5 / 0.5)  # prior
    G[c] = G[c] + l_c - l_free
...
The optimization of factor graphs is performed using iterative nonlinear least squares solvers. Gauss-Newton and Levenberg-Marquardt algorithms linearize the error terms around the current estimate and solve the resulting sparse linear system H · ΔX = -b. The Hessian matrix H = J^T Σ^{-1} J inherits the sparsity pattern of the factor graph, enabling efficient solution using sparse Cholesky decomposition (O(N log N) for typical underwater graphs). iSAM2 (Kaess et al., 2012) introduces incremental optimization using the Bayes tree data structure, which updates only the affected portion of the graph when new measurements arrive, achieving real-time performance for large-scale problems.

Loop closure detection is critical for correcting drift in underwater SLAM. Acoustic loop closure relies on recognizing previously visited locations from sonar imagery. Traditional approaches use bag-of-words (BoW) with handcrafted features (SIFT, SURF adapted for sonar), while recent methods employ learned descriptors from convolutional neural networks. Hidalgo-Carrió et al. (2020) demonstrated that contrastive learning on sonar image pairs achieves 85% recall at 95% precision for place recognition in underwater environments. Once a loop closure is hypothesized, the relative transform between the two poses is estimated using ICP (Iterative Closest Point) or NDT (Normal Distributions Transform) on the sonar point clouds.
...
For each timestep k:
    ...
    # Add odometry factor (from IMU/DVL)
    G.add_factor(BetweenFactor(x_{k-1}, x_k, u_k, Sigma_odo))
    
    # Add observation factors (from sonar)
...
for iteration = 1 to max_iterations:
    ...
    # Line search (Levenberg-Marquardt damping)
    X_new = X + Delta
...
```
Input: Current sonar image I_k, Database of previous images {I_1, ..., I_{k-1}}

# Feature extraction
features_k = extract_features(I_k)  # CNN or handcrafted
...
Source: Kaess et al. (2012), Table I; Kümmerle et al. (2011), Table II.
...
# Compute covariance (linearized propagation)
...
```
Input: All sensor measurements, initial state
...
```
Input: Current sensor status flags

# Detect dropouts
if DVL_status == LOCK_LOST:
    # Use IMU-only propagation with increased uncertainty
    Sigma_DVL = INF  # effectively disable DVL factor
    
    # If sonar available, use sonar to bound drift
    if sonar_features_available:
        increase_sonar_weight
    
    # If no sensors available, enter dead reckoning mode
    if sonar_status == NO_FEATURES:
        activate_drift_warning
        use_motion_model_only

if sonar_status == INTERFERENCE:
    # Reject sonar measurements with high innovation
    for each sonar measurement:
        if innovation > 3 * sqrt(S):
            reject_measurement
    
    # Fall back to visual SLAM if camera available
    if camera_available:
        activate_visual_odometry

# Recover when sensor returns
if DVL_status == LOCK_ACQUIRED:
    # Reset DVL factor with nominal covariance
    Sigma_DVL = nominal_value
    
    # Perform local optimization to correct accumulated drift
...
# Align trajectories (Sim(3) alignment)
...
# Relative Pose Error (RPE) with delta = 1s
RPE = 0
count = 0
for t = 1 to T - delta:
    # Relative transform from t to t+delta
